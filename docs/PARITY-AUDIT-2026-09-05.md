...
| --- | --- |
| Short-tap primary barrel rolls | Present captured shoulder press edges for one native gameplay update and retain releases between sampled updates. New presentation-sampled replays cover single/paired 1 ms taps, both shoulders, both paces, routes 2/3 and four output schedules. The prior bridge fails an isolated paired-tap replay with player control enabled in both ports. EX secondary/multitap and mirror-mode rolls also pass 24 native-player short-tap cases. Repeated presses received in separate presentation samples are retained across pending updates; complete taps within a neutral-start SDL event batch are also counted. Source-aware keyboard/gamepad batches also preserve release/repress of previously held actions; cross-button timestamp ordering and analog/touch excursions remain outside this correction. See INPUT-PACING-VALIDATION.md. |
| Digital taps between presentations | SDL virtual-gamepad evidence reproduces lost complete shoulder taps in final-state-only sampling. Preserve mapped digital press/release events alongside held samples; desktop and UWP input checks pass. Primary and EX secondary held-transition rolls now have presentation-sampled/native-strategy regressions. See INPUT-PACING-VALIDATION.md. |
| Gameplay bitmap DMA / OAM | Correct ordinary completion from 300 to 328 OAM bytes, verified against both native IRQBIT3 DMA blocks. Expose separate NTSC bitmap DMA phases and their completion gate; native IRQBIT1/2 VRAM comparisons pass. An optional native instruction-boundary callback can own DMA phase advancement and preserve intermediate values across calls/resumes. Gameplay has not installed a raster scheduler yet, so live transfer timing remains open. See STAGE-STATE-PARITY-VALIDATION.md. |
