...
| Post-0.0.4 continuous stage-state comparison | IRQ flash palettes/Original RNG and premature background-map continuation corrected. All 19 Original and 39/40 EX numbered openings pass 300 updates; the three corrected cases also pass 1,000 updates each. EX LEVEL7_2 still fails on SCORPION4's volatile TRANS_FLAG read. A native-only trace observes both 2 and 4 at that instruction, so a fixed replacement is insufficient. Full CPU/IRQ/GSU scheduling remains open; see STAGE-STATE-PARITY-VALIDATION.md. |
...
| Native CPU parity audit | Corrected accumulator restoration, decimal and wrapped-zero arithmetic, operand-bank wrapping, read-modify-write order, block transfers, XCE and instruction clocks. Independent Ares comparisons pass 12,192 opcode cases, 524,288 decimal cases and 1,048,576 native arithmetic routines. CPU-PARITY-VALIDATION.md records scope; these partial CPU clocks are not yet the Original-pace scheduler. |
| Native camera and crosshair follow-up | Corrected WMAT11/WMAT11W high-byte/word aliasing in the runtime and the earlier reference fixture. GETVIEW_L now owns the camera and original integer crosshair projection directly. The corrected reference exposes 2,772 old mismatches and matches all 12,204 object views afterward. The full 61-test suite and nine final rebuild checks pass. Full-system camera comparisons and hardware-profile limits are recorded in CAMERA-PARITY-VALIDATION.md. |
