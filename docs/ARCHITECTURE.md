...
## Why 60 Hz cannot drive gameplay
...
The timing code uses integer nanosecond phase units, not a floating-point
`1/20` accumulator. Floating-point values exist only in the rendered transform.
The simulation remains fixed-point and deterministic.
...
interpreter and rasterizer. It renders the original 224x192 viewport into an
indexed-color framebuffer, including source face order, fixed-point matrices,
near/screen clipping, polygon and texture scan conversion, line rules,
sprite-faces, shadows, LOD selection, lighting, and depth palettes.
...
priority semantics out of the model decoder and allows the 60 Hz presentation
loop to interpolate only world/camera transforms.
...
The two interpolated frames between boundaries are validated for endpoint
identity and monotonic transform motion; they are new presentation frames and
cannot have an original framebuffer equivalent.
