...
inline std::array<float,3> environment_colour(std::array<float,3> c,unsigned kind,
    ...
    if(kind<=5 && m[0]) {
        ...
        if(kind<=4) {
            // One broad, world-anchored noise field is enough once the 3D
            // terrain supplies relief. The former high-frequency grain was
            // costly and crawled visibly during travel.
            const float a=environment_noise(u*.35f,z*.35f)*2-1;
            if(kind==1) {detail=a*(.07f+.02f*near);tint={.91f,1.06f,.86f};}
            if(kind==2) {detail=a*(.055f+.015f*near);tint={1.f,1.f,1.f};}
            if(kind==3) {
                detail=a*.05f+std::sin(z*.48f+a*2.f)*.035f*near;
                tint={1.12f,1.04f,.89f};
            }
            if(kind==4) {detail=a*(.035f+.01f*near);tint={1.03f,1.06f,1.12f};}
        }
        if(kind==5) {
...
inline void apply_environment(const EnvironmentEffects& e,const Framebuffer& frame,std::vector<std::uint8_t>& rgba,RowWorkers* workers=nullptr) {
    ...
    for(std::size_t i=std::size_t(first)*width;i<std::size_t(last)*width;++i) {
        ...
        if(!terrain && !kind && e.modes[0]==1 && e.plane[1]>=1 && e.plane[1]<=4) {
            const float distance=y-e.motion[0]-e.plane[0]*x;
            if(distance>=0 && distance<32) kind=unsigned(e.plane[1]);
        }
        if(!kind) continue;
        if(terrain) continue; // Mesh colour/normals are world-anchored already.
