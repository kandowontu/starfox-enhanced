...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    if(stage==31) {
        ...
        if(!terrain && kind==0 && layer==2 && environmentModes.x==1
            ...
            if(distance>=0 && distance<32) kind=uint(environmentPlane.y);
        }
        if((layer==2 || terrain) && kind!=0) {
            // Terrain's mesh normals and palette already supply world-space
            // relief. Screen-space procedural noise made it shimmer during
            // travel and redundantly shaded every ground pixel.
            if(terrain) { outputImage[p]=float4(result)/255; return; }
            float x=(float(p.x)+.5)/float(scale)-float(width)/float(scale)*.5;
            float y=(float(p.y)+.5)/float(scale);
            uint4 modes=environmentModes;float3 base=float3(c.rgb);
            float3 upgraded=environmentColour(base,kind,x,y,modes,environmentMotion,environmentPlane.x);
