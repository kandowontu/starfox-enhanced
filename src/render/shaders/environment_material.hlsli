...
float3 environmentColour(float3 c,uint kind,float x,float y,uint4 m,float4 v,float slope) {
    ...
    if(kind<=5 && m[0]) {
        ...
        if(kind<=4) {
            ...
            if(kind==1) {detail=a*(.07f+.02f*near);tint=float3(.91f,1.06f,.86f);}
            if(kind==2) {detail=a*(.055f+.015f*near);tint=float3(1,1,1);}
            if(kind==3) {
                detail=a*.05f+sin(z*.48f+a*2.f)*.035f*near;
                tint=float3(1.12f,1.04f,.89f);
            }
            if(kind==4) {detail=a*(.035f+.01f*near);tint=float3(1.03f,1.06f,1.12f);}
        }
        if(kind==5) {
