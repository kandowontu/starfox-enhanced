    ...
    try {
        ...
        for(const auto& draw:draws) {
            if(const auto* model=std::get_if<GpuModelDraw>(&draw)) {
                ...
                if(model->ray_geometry && !model->emissive) {
                    rays_requested=true;
        ...
        for(const auto& draw:draws) {
            ...
            if(const auto* model=std::get_if<GpuModelDraw>(&draw)) {
                ...
                front=renderer.enqueue(device,command,*model->shape,pose,model->settings,
                    ...
                    encode_rays && model->ray_geometry && !model->emissive?&rays:nullptr,previous?&*previous:nullptr,model_jitter,
                ...
                if(encode_rays && model->ray_geometry && !model->emissive) {
                    if(!rays.points) {
