...
Vec3 explosion_offset(
    ...
    const auto expand = [progress = pose.explosion_phase.value_or(
        double(pose.explosion_progress))](double value) {
        const auto rounded = static_cast<std::int32_t>(std::lround(value));
        const auto low = static_cast<std::int32_t>(std::floor(progress));
        const auto high = static_cast<std::int32_t>(std::ceil(progress));
        const auto offset = [rounded](std::int32_t count) {
            return starfox::simulation::arithmetic_shift_right(rounded*count,2U);
        };
        return std::lerp(double(offset(low)),double(offset(high)),
            progress-double(low));
    };
